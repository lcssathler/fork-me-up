...
M2-S01 and M2-S02 establish the first real local-source read boundary without treating configuration-time path approval as permanent authority. The collector accepts only an in-process authority issued by the resolver; re-canonicalizes roots, repositories, directories, and files at use; verifies containment against both selected repository and authorized root; rejects links, special files, replacement, unsafe names, and nested repositories; and bounds enumeration, file size, total bytes, depth, and elapsed time. Only supported regular UTF-8 text is opened. Unsupported files, ignored state, archives, dependencies, and nested-repository contents are never opened or expanded, and no repository command, hook, filter, package script, URL, network primitive, or executable is invoked.

The resulting snapshot is private pre-Evidence metadata. It contains hashes and therefore remains sensitive, but no raw text, heading text, source snippet, package name/version/value, script value, absolute path, native diagnostic, stack, or partial result. Binary, invalid UTF-8, malformed selected JSON, budget exhaustion, authorization change, and filesystem failures return fixed category/retryability only. Synthetic tests cover forged authority, path replacement and escape, real junction/symlink rejection, canaries, binary/invalid content, nested and ignored trees, deterministic ordering, immutability, and count/file/total/depth/time limits. Git safety, attribution, evidence quality, persistence, owner controls, and private-source operation remain later gates.
