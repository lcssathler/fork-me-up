...
The resulting snapshots are private pre-Evidence metadata. They contain hashes and therefore remain sensitive, but no raw text, heading text, source snippet, package value, raw identity or commit message, absolute path, native diagnostic, stack, or partial result. Invalid content or metadata, budget exhaustion, authorization change, and filesystem/command failures return fixed category/retryability only. Identity attribution, bot/human decisions, evidence quality, persistence, owner controls, and private-source operation remain later gates.
