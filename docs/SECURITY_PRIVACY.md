...
M2-S13 keeps owner source authority separate from read-only consumer requests. Explicit configuration files are bounded, regular-file checked and reread with identity/stamp/canonical checks; no discovery or code execution occurs. Store/source overlap is rejected before runtime creation. Malformed owner commands do not invalidate usable state; an accepted incomplete refresh preserves the Store while withholding that process's aggregate context. Store-backed Provider calls reload protected state and apply stale flags in a private copy, without writes or collection. The MCP transport awaits asynchronous responses and converts rejected invocations into fixed errors. Full synthetic CLI/MCP integration verifies actual Git collection, cache reuse, correction/refresh/restart, minimized delivery, diagnostics, export, deletion and untouched source. [Local workflow](LOCAL_COMMUNITY.md) documents private configuration, consumer trust and degraded-state limits.
...
M0-S08 adds development-only [DCP draft checks](PROTOCOL.md#42-draft-limits-and-validation-scope): exact authoring fields, bounded fixture reads, invalid-input and symlink cases, and fixed CLI diagnostics. M0-S09 extends that bounded offline path to the independent Evidence and Claim contracts, including observation/claim separation, safe source-relative syntax, authorship uncertainty, timestamp ordering, state/basis matching, required automated/declaration/dispute provenance, project scope, and rejection of raw or policy-bearing fields. M0-S10 adds Store/Export/DCP envelope separation, explicit export exclusions, local-only schema dependency loading, nested Evidence validation, in-envelope reference integrity, and Store timestamp ordering. M0-S11 adds Demand/DCP/Profile-envelope separation, bounded task text, typed task/project capability basis, metadata-availability consistency, and rejection of raw project, profile, authority, and policy fields. M0-S12 adds content-free Provider capability discovery/errors, typed operation subsets, request correlation, version and limit checks, bounded metadata, and namespaced extensions while rejecting credentials, arbitrary subjects, collector configuration, raw data, policy, and client-specific fields. M1-S02 adds pure behavior-precedence tests that derive policy only from typed Claim states, structured adjacency, and validated preferences; instruction-like rationale and limitations remain inert data. M1-S03 validates Demand input at runtime, selects only exact global or current-project Claims, exposes unmatched demand without inventing Claims, omits complete profile/evidence records, and returns content-free invalid-input errors; task text remains inert. M1-S04 validates the emitted DCP, fails closed on denied authorization/redaction/budget, omits full profile and Evidence records, replaces canary/secret/path-bearing free text, and proves task prose cannot change policy under strict byte/token-accounting limits. These tests do not prove source authorization, identity, demand or claim derivation, persisted correction precedence, atomic persistence, migration, owner intent, successful export/deletion, real grant resolution, pairwise identity, transport security, or safe adapter behavior. Runtime provider and integrated behavioral conformance gates remain mandatory.

M3-S03 adds consumer-boundary coverage for every Claim state, exact Response Policy retention, expiry, local/external audience mismatch, unsupported versions, malformed and oversized input, immutability, fixed diagnostics, and free-text canary exclusion. Artifact integration remains serialized with the existing Protocol builder: one tarball is installed through a derived lock and `npm ci --offline --ignore-scripts`, then the generic process imports only package exports. `FMU-E-016` compares its structured projection with Codex for the same fixture; model prose, authorization and untested clients remain outside the claim.

M2-S01 through M2-S05 establish the first real local-source read and pre-Evidence assessment boundaries without treating configuration-time path approval as permanent authority. Both collectors accept only an in-process authority issued by the resolver and re-canonicalize the selected root and repository at use. Filesystem collection additionally rechecks every directory/file, rejects links, special files, replacement, unsafe names, and nested repositories, and bounds enumeration, file size, total bytes, depth, and elapsed time. Only supported regular UTF-8 text is opened; ignored state, archives, dependencies, and nested-repository contents are never opened or expanded, and no repository executable is invoked.

Git collection supports only standard contained `.git` and object directories. It parses bounded `HEAD`, branch/packed-ref, and shallow metadata directly, rejects links, special objects and alternates, fingerprints the object tree before and after commands, and runs only exact plumbing argument arrays in a fresh trusted bare quarantine. The original repository configuration and worktree are never command inputs; system/global/local config, attributes, hooks, filters, external diff/textconv, pagers, replacement objects, lazy fetch, prompts, credentials, remotes, shell evaluation, and source writes are disabled or structurally excluded. SHA-1/SHA-256 histories, object/path/output/deadline limits, hostile controls, malformed config, canaries, merges, coauthors, truncation, packed and shallow heads, Git failures, and object-store escape/exhaustion are tested using only temporary synthetic repositories.

The resulting collector, authorship, and source-risk snapshots are private pre-Evidence metadata. They contain hashes and therefore remain sensitive, but no raw text, heading text, source snippet, package value, raw identity or commit message, absolute path, native diagnostic, stack, or partial result. Invalid content, identity/risk configuration, metadata or annotation, budget exhaustion, authorization change, snapshot mismatch, and filesystem/command failures return fixed category/retryability only. Synthetic assessment tests cover normalization, duplicate/forged configuration, every attribution/history/source-risk state, bot/coauthor/committer limitations, SHA identifier formats, malformed snapshots, dangling annotations, deterministic immutable results, raw-identity/content canaries, and real collector compatibility. `FMU-E-008` proves shared and unknown authorship cannot establish high-confidence personal depth; `FMU-E-007` proves risky source classes are visibly down-ranked and cannot independently establish demonstrated depth. Owner controls and private-source orchestration remain later gates.

M2-S06 adds the first profile persistence boundary without connecting it to source collection or consumers. Store configuration and every outcome are content-free; profile data remains only in the private configured directory and immutable returned value. Unit fault injection covers failures before and after activation and cleanup, while temporary-directory integration covers real readback, hard-link compare-and-set concurrency, rollback retention, corruption recovery, orphan staging, migration, and filesystem redirection. `FMU-E-011` confirms that only a validated reread reports commitment and a rejected write leaves the prior valid profile loadable. Correction precedence, owner operations, export/deletion, Provider integration, encryption, backup, and multi-user isolation remain later gates.

M2-S07 connects no source or consumer itself: its pure input is the already sanitized authentic M2-S05 snapshot. Unit coverage exercises authorship/risk ceilings, exact request and graph validation, bounds, stale behavior, every invalidation reason, forged snapshots, backward time, mapping mismatches, canaries, immutability, and absence of filesystem/process/network/model/logging authority. Temporary-repository integration runs the real M2 chain through Store-compatible serialization and a changed-source refresh without emitting source content, identity, commit prose, or absolute paths. Correction precedence, owner controls, cache-origin orchestration, persistence workflow, Provider integration, and broader taxonomy inference remain later gates.

M2-S08 authenticates already-collected immutable filesystem metadata without adding a filesystem capability. Exact versioned JSON requests, repository/file correspondence, byte/count limits, and final Protocol validation fail closed. Only the selected repository's source languages and explicit typed task capabilities can enter demand; repository prose, dependencies, scripts and private paths cannot affect policy or be reflected in output. A fixed summary replaces task prose. Clarification choices carry only bounded typed capability and operation metadata; risk labels never grant permission. Forged pending state or an absent/unoffered answer returns no demand, and missing source metadata remains explicit. Synthetic unit, real collection-to-Core integration, and `FMU-E-009`/`FMU-E-010` coverage verify source isolation, immutable output, canary absence and bounded clarification. Generated time and revision hashes do not establish metadata freshness or sharing authorization. See [ADR-0025](adr/0025-source-backed-demand-profile-producer.md).
