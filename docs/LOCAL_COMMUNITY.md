...
GitHub access requires a previously authenticated `gh` CLI installation. Fork Me Up does not prompt for login, accept a token or credential path in this configuration, create credentials, or persist authentication. It issues fixed read-only `github.com` metadata requests only during an owner refresh, verifies that the mapped repository is public and performs no automatic retry. Consent expiry is checked against the runtime clock, so an older refresh `observedAt` cannot extend the window. Local Git time is charged first, so the effective remote timeout is the smaller of `publicHistory.github.limits.maxDurationMs` and the remaining refresh deadline. Complete local Git history always wins and produces zero network requests. An expired consent block also produces no request; remove the entire block to opt out immediately for the next process start. MCP consumers cannot start collection or network access.
