...
| 5 | `M0-S07` — Repository governance policies | `Ready` | Contribution, vulnerability-reporting, changelog, and versioning policies reference the actual verification path and are in place before external contributions are accepted. | M0 governance deliverable and gate; no behavioral evaluation applies. |
...
ADR-0006 records the formatter, lint, strict TypeScript, built-in test runner, aggregate-check, and least-privilege CI choices. Unit tests are non-empty; integration and evaluation commands use explicit `bootstrap-not-applicable` entries only until their named product boundaries exist. The aggregate check remains deterministic and excludes live audit, schema validation, documentation links, secret scanning, dependency review, and other later gates.

No `FMU-E-*` evaluation applies because M0-S05 establishes development and CI controls rather than product behavior. After M0-S05 is integrated into `main`, the immediate next action is M0-S07, repository governance policies. Any behavioral spike before the M0 exit gate is disposable and cannot enter product code.
