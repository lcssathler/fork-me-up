...
| 2 | `M0-S02` — Basic remote branch protection | `Owner decision required` | Authorized remote verification found that the current private-repository plan does not expose branch protection or rulesets, so `main` remains unprotected. The owner must choose whether to enable a supporting GitHub plan or approve development of a specifically documented candidate equivalent that can enforce reviewed pull requests and prevent force pushes and deletion. The decision alone does not complete this slice. | M0 Git deliverable; external-effect rules in `AGENTS.md` and `ENGINEERING.md`. |
| 3 | `M0-S03` — License and trademark decision | `Blocked by M0-S02` | The owner completes the required legal/trademark review, explicitly accepts the public-code license and trademark policy and their Community/Cloud implications, and adds the corresponding license and policy files before any public code or package release. | M0 license deliverable and exit gate; Section 11; ADR-0001 or an accepted successor. A pending review cannot complete this slice. After M0-S02 reaches `Complete` and that transition integrates into `main`, change state to `Owner decision required`. |
...
- no formally accepted equivalent control exists, and required status checks remain `bootstrap-not-applicable` until `M0-S06`.

Public visibility is not an eligible workaround before the `M0-S03` license and trademark decision. `M0-S02` therefore remains incomplete. No `FMU-E-*` evaluation applies because this slice changes repository governance rather than product behavior.

The owner decision selects an implementation path; it does not authorize a billing change or later remote configuration unless the same active task explicitly authorizes that exact effect. After the selected path is defined and accepted, set `M0-S02` to `External authorization required` if it still needs an external mutation, or to `Ready` if it can be implemented entirely within the repository. An alternative qualifies as equivalent only after it is documented, implemented, and verified to require reviewed pull requests and prevent force pushes and deletion. `M0-S02` becomes `Complete` only after the selected control is configured, verified, reviewed, and integrated into `main`.

The immediate next action is the `M0-S02` owner decision: enable a GitHub plan that supports protection for this private repository or approve development of a specifically documented candidate equivalent. No implementation slice is currently `Ready`; `M0-S03` remains blocked. Any behavioral spike before the M0 exit gate is disposable and cannot enter product code.
