...
| 6 | `M1-S06` — Reference adapter and graceful degradation | `Ready` | One reference adapter maps only allowlisted response-policy fields, exercises task/session delivery, preserves ordinary host work when unavailable, and adds compaction restoration only if the selected client capability supports it. | M1 adapter and degraded-operation deliverables; `FMU-FR-012`, `FMU-FR-025`; `FMU-NFR-001`, `FMU-NFR-008`; `FMU-E-014`; ADR-0002. |
...
FMU-E-001 through FMU-E-004 now execute against the structured Core output: demonstrated Java selects concise intent; insufficient CI evidence selects purpose, result, risk, and rollback guidance; Angular evidence permits only an explicit bounded React analogy; and absence remains `insufficient-evidence` rather than ignorance. These evaluations prove policy intent, not prose generated by an adapter or model. Integrated consumer behavior remains an M1-S06/M1-S07 gate.
