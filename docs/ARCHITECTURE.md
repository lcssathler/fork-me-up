...
M2-S01 through M2-S05 keep local source access and pre-Evidence assessment inside `packages/community-provider`, below owner workflows and outside Protocol/Core/MCP payloads. Configuration issues a private live authority for canonical selected repositories. The filesystem collector accepts only that authority, then rechecks canonical path identity and containment for every root, repository, directory, and selected file at use. Its injected Node port can inspect paths, probe only the fixed nested-repository marker, enumerate bounded names, and read regular files through bounded read-only handles. See [ADR-0018](adr/0018-authorized-local-repository-configuration.md), [ADR-0019](adr/0019-bounded-filesystem-metadata-collector.md), [ADR-0020](adr/0020-quarantined-bounded-git-metadata-collector.md), [ADR-0021](adr/0021-explicit-conservative-git-authorship-assessment.md), and [ADR-0022](adr/0022-conservative-evidence-source-risk-classification.md).

Collection is sequential and all-or-nothing. Fixed version-control, dependency, and local-state directories are ignored by filesystem traversal; a nested `.git` marker ends traversal at that directory; unsupported files are counted but not opened; links and special files fail closed. Supported UTF-8 documents, manifests, and source files become one immutable private snapshot containing only repository-relative paths, SHA-256 digests, byte/line/structure counts, fixed formats/languages, test markers, and bounded sanitized `package.json` names.

Git collection separately resolves only bounded `HEAD`, branch/packed-ref, and shallow control data, validates the complete selected object-store metadata before and after use, and rejects alternate or linked object databases. Each exact allowlisted plumbing command runs with `shell: false` against the authorized object directory from a new trusted temporary bare Git directory, so repository configuration, worktree attributes, hooks, filters, external diff, text conversion, pagers, replacement objects, lazy fetching, remotes, and credentials are outside the command boundary. The resulting private snapshot contains only SHA-1/SHA-256 object IDs, parent relationships, UTC timestamps, SHA-256 identity/coauthor digests, and safe changed paths under explicit commit/object/path/output/time ceilings. Raw source, absolute paths, manifest values, raw identities or messages, native diagnostics, Evidence, Claims, and policy do not cross either collector boundary.
