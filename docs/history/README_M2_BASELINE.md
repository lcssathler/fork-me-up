# Historical README at M2 completion

> Preserved from main cc93ae9. This is a dated development description, not current setup or routing guidance. Use the [current README](../../README.md).
...
The [core evidence method](../EVIDENCE_METHOD.md) explains what Git and source observations can support, how AI-assisted contributions are treated, and what remains uncertain. Generated-artifact indicators do not detect AI or measure human knowledge. The [frozen M2 quality experiment](../evaluations/M2_QUALITY_PROTOCOL.md) tests those rules without claiming real-world skill accuracy.
...
M0, M1, and M2 are complete, and M3 is the next planned milestone. The repository is public, and `main` requires pull requests and the GitHub Actions `Windows baseline` check while blocking deletion and non-fast-forward updates. Private unreleased Protocol, Core, Community Provider, MCP, and Codex adapter workspaces now support a bounded [local Community workflow](../LOCAL_COMMUNITY.md): explicitly selected local repositories are collected without executing their code; conservative Evidence and Claims are persisted atomically; the owner can inspect, correct, diagnose, export, import, and delete managed data; and a Store-backed local MCP process delivers minimized task context. The [researched evidence method](../EVIDENCE_METHOD.md), [frozen quality measurement](../evaluations/M2_QUALITY_RESULTS.md), and [M2 exit audit](../audits/M2_EXIT_AUDIT.md) record the evidence and limits. There is still no product/package release, second materially different consumer, macOS/Linux support claim, or network source connector; those remain gated M3 work.
...
The root manifest declares native npm workspaces under `packages/*`, `apps/*`, and `adapters/*`. The private unreleased `@fork-me-up/protocol`, `@fork-me-up/core`, `@fork-me-up/community-provider`, `@fork-me-up/mcp-local`, and `@fork-me-up/codex-adapter` workspaces implement the current local development runtime. Protocol validates canonical exchange contracts, Core owns pure domain behavior, the Community Provider maps typed requests into bounded collection, persistence, owner control, and compilation, the MCP application maps the read-only consumer surface onto `stdio`, and the Codex adapter maps only allowlisted fields into client-owned hook output. The permitted dependency direction is applications and reference adapters → Community provider → Core → Protocol. Public packages must never import proprietary Cloud/Pro modules.
...
Development-only fixture selections are `demonstrated`, `adjacent`, `insufficient-evidence`, and `unavailable`; for example, `npm run mcp -- --fixture=adjacent`. The process speaks newline-delimited JSON-RPC on `stdin`/`stdout`, advertises only `get_task_context` and `get_profile_metadata`, and intentionally supports MCP revision `2025-11-25` only. It opens no listener and makes no network request. These fixture modes prove the transport boundary; the [local Community workflow](../LOCAL_COMMUNITY.md) documents the separate Store-backed mode. Neither mode proves compatibility with every MCP client or represents a packaged release.
...
The aggregate command fails fast across formatting, lint, strict type checking, unit tests, all draft schema and provider/conformance fixtures, real local source/Store/owner/MCP/Codex subprocess integration tests, the frozen M2 measurement, and behavioral evaluations. Unit, integration, and evaluation suites must never be empty. The evaluation suite covers FMU-E-001 through FMU-E-015 where applicable; FMU-E-016 and a materially different second consumer remain M3 gates. The separate `npm run measure:m2 -- --output <new-report-path.json>` command reproduces the frozen limited-taxonomy quality experiment from a full Git checkout.
...
This baseline is not the complete release gate. Draft contract validation runs through `npm run schema:check`, and GitGuardian provides the current pull-request secret check. Documentation-link, dependency/vulnerability, and license review are still recorded explicitly when applicable; automated cross-platform install/uninstall, package inspection, SBOM, checksums, and provenance/signing remain M3 gates. See the [draft contract and limits](../PROTOCOL.md#41-exact-draft-authoring-rules) and [object boundaries](../PROTOCOL.md#2-objects-and-boundaries). The root package stays private to prevent accidental publication, while all repository content remains licensed under Apache-2.0.
...
- [Roadmap](../ROADMAP.md)
- [M0 exit audit](../audits/M0_EXIT_AUDIT.md)
- [M1 exit audit](../audits/M1_EXIT_AUDIT.md)
- [M2 exit audit](../audits/M2_EXIT_AUDIT.md)
