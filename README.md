...
M0, M1, and M2 are complete, and M3 is the next planned milestone. The repository is public, and `main` requires pull requests and the GitHub Actions `Windows baseline` check while blocking deletion and non-fast-forward updates. Private unreleased Protocol, Core, Community Provider, MCP, and Codex adapter workspaces now support a bounded [local Community workflow](docs/LOCAL_COMMUNITY.md): explicitly selected local repositories are collected without executing their code; conservative Evidence and Claims are persisted atomically; the owner can inspect, correct, diagnose, export, import, and delete managed data; and a Store-backed local MCP process delivers minimized task context. The [researched evidence method](docs/EVIDENCE_METHOD.md), [frozen quality measurement](docs/evaluations/M2_QUALITY_RESULTS.md), and [M2 exit audit](docs/audits/M2_EXIT_AUDIT.md) record the evidence and limits. There is still no product/package release, second materially different consumer, macOS/Linux support claim, or network source connector; those remain gated M3 work.
...
- [M2 exit audit](docs/audits/M2_EXIT_AUDIT.md)
