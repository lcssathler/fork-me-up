...
The Community Provider also owns the M2 implementation-internal local source boundaries. Closed configuration resolves explicitly selected directories, rejects physical escape or duplicate aliases, and retains absolute paths only in the private owner-side authority. A sequential collector then recanonicalizes roots, repositories, directories, and files; ignores version-control/dependency state and nested repositories; reads only a fixed set of bounded UTF-8 documents, manifests, and source files; and emits relative paths, hashes, sanitized manifest names, and structural counts without raw text. Links, special files, invalid content, replacement, exhaustion, or filesystem failure abort the complete private snapshot with a content-free error. Git collection begins in M2-S03; none of these internal objects are Profile Provider or Protocol payloads.
