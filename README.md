...
M0 and M1 are complete, and M2 is in progress. The repository is public, and `main` requires pull requests and the GitHub Actions `Windows baseline` check while blocking deletion and non-fast-forward updates. Minimal private Protocol, Core, Community Provider, MCP, and Codex adapter workspaces validate synthetic fixture profiles, compile schema-valid redacted DCPs, and deliver allowlisted task guidance through non-blocking Codex lifecycle hooks. The [M1 exit audit](docs/audits/M1_EXIT_AUDIT.md) verifies the fixture-backed technical MVP and records its limits. M2 now has bounded filesystem/Git collectors, conservative authorship and source-risk assessment, atomic local profile storage, deterministic Evidence/Claim derivation, and source-backed Demand Profile production with one material clarification. M2-S09 adds bounded incremental multi-repository refresh with an ephemeral source cache, preserved observation age and explicit degraded states. M2-S10 adds a [local owner CLI](docs/OWNER_WORKFLOW.md) for inspecting, declaring, correcting, disputing and rejecting Claims with verified persistence and preserved history. M2-S11 adds redacted owner import/export and verified local Store/cache deletion with explicit recovery and retained deletion barriers. M2-S12 adds bounded owner evidence lookup and read-only diagnostics for installation, Store schema/gates, adapter cache and context size. M2-S13 connects these components through a [complete local workflow](docs/LOCAL_COMMUNITY.md) and Store-backed MCP delivery. M2-S14 documents the [researched evidence method](docs/EVIDENCE_METHOD.md), and M2-S15 adds a [reproducible frozen quality measurement](docs/evaluations/M2_QUALITY_RESULTS.md). The [machine handoff](docs/handoffs/M2_NEXT_MACHINE.md) records the stop after S15 and how to resume. The M2 exit audit, persistent source cache, release, a second consumer, and broader client compatibility remain later gates.
