...
M0 and M1 are complete, and M2 is in progress. The repository is public, and `main` requires pull requests and the GitHub Actions `Windows baseline` check while blocking deletion and non-fast-forward updates. Minimal private Protocol, Core, Community Provider, MCP, and Codex adapter workspaces validate synthetic fixture profiles, compile schema-valid redacted DCPs, and deliver allowlisted task guidance through non-blocking Codex lifecycle hooks. The [M1 exit audit](docs/audits/M1_EXIT_AUDIT.md) verifies the fixture-backed technical MVP and records its limits. M2 now has bounded filesystem/Git collectors, conservative authorship and source-risk assessment, atomic local profile storage, deterministic Evidence/Claim derivation, and source-backed Demand Profile production with one material clarification. M2-S09 adds bounded incremental multi-repository refresh with an ephemeral source cache, preserved observation age and explicit degraded states. Owner workflows, persistent cache, release, a second consumer, and broader client compatibility remain later gates.
